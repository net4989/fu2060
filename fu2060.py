...
class MyWindow(Layout):
    def __init__(self):
        ...
        self.setWindowTitle('fu2060(선물 트레이딩 & 옵션 매도 헷지)')
...
# _opw00018(계좌평가잔고내역요청)
    ...
    def _opw20009(self, rqname, trcode):
        ...
        self.printt('self.order_able_cash')
        self.printt(self.order_able_cash)
...
# 이벤트 슬롯
    ...
    def test(self):
        ...
        day_residue_int = 3
        self.futrue_s_data['day_residue'][0] = 1
        self.option_myhave = {'code': [], 'myhave_cnt': [], 'sell_or_buy': []}
        # self.option_myhave = {'code': ['101V3000', '301V2337', '201V2342'],
        #                       'myhave_cnt': [2, 3, 5],
        #                       'sell_or_buy': [2, 1, 1]}
        # self.option_myhave = {'code': ['101V3000'], 'myhave_cnt': [0], 'sell_or_buy': [1]}
        # -----


        # 현재가 : 341.3
        self.stock_trend_line_of_ai_day = {'stock_no': ['10100000'],
         'poly_sell_max_price': [340.7283139259415],
         'poly_buy_min_price': [340.4001876348584],
         'sell_max_price': [365.3891729323308],
         'buy_min_price': [339.617289524868],
         'poly_h_gradient': [-0.008352946615162912],
         'poly_l_gradient': [0.0007199808014179739],
         'h_gradient': [-0.0012030075187973485],
         'l_gradient': [0.6672895248680191]}
...
# 주문
    ...
    # 옵션 종목검색
    def option_items_search(self, myhave_total_future_s_cnt, put_or_call, month_mall_type):
        # -----
        # 옵션 임시재고
        ...
        # 현재 선물의 델타를 건수 * 100 곱해서 절대값 구하고
        myhave_future_s_delta_sum = abs(myhave_total_future_s_cnt * 100)
        ...
        if put_or_call == 'put':
            ...
            if month_mall_type == 'center_index':
                ...
                for mh in range(len(self.option_myhave['code'])):
                    # 풋이므로 +
                    # 당월물
                    for op in range(self.center_index + 2, self.center_index - 1, -1):
                ...
                # 옵션의 델타가 선물의 델타보다 클때 [옵션 청산 매수]
                    ...
                    # 옵션과 선물의 델타 차이를 비교하여 청산(매수)옵션의 델타보가 클때만 [풋이므로 +]
                    if myhave_option_s_delta_diff > abs(self.output_put_option_data['Delta'][self.center_index + 2]):
                        # 전체 보유 옵션을 돌리고
                        for mh in range(len(self.option_myhave['code'])):
                            # 풋일때만
                            if self.option_myhave['code'][mh][:3] == '301':
            ...
            # 차월물
                ...
                for mh in range(len(self.option_myhave['code'])):
                    # 풋이므로 +
                    # 차월물
                    for op in range(self.center_index_45 + 2, self.center_index_45 - 1, -1):
                        if self.option_myhave['code'][mh][:3] == '301':
                            if self.option_myhave['sell_or_buy'][mh] == 1:
                ...
                # 옵션의 델타가 선물의 델타보다 클때 [옵션 청산 매수]
                    ...
                    myhave_option_s_delta_diff = myhave_option_s_delta - myhave_future_s_delta_sum
                    # 옵션과 선물의 델타 차이를 비교하여 청산(매수)옵션의 델타보가 클때만 [풋이므로 +]
                    if myhave_option_s_delta_diff > abs(self.output_put_option_data_45['Delta'][self.center_index_45 + 2]):
                        # 전체 보유 옵션을 돌리고
                        for mh in range(len(self.option_myhave['code'])):
                            # 풋일때만
                            if self.option_myhave['code'][mh][:3] == '301':
        ...
        # call
            ...
            if month_mall_type == 'center_index':
                ...
                # 옵션의 델타가 선물의 델타보다 클때 [옵션 청산 매수]
                    ...
                    myhave_option_s_delta_diff = myhave_option_s_delta - myhave_future_s_delta_sum
                    # 옵션과 선물의 델타 차이를 비교하여 청산(매수)옵션의 델타보가 클때만 [풋이므로 +]
                    if myhave_option_s_delta_diff > abs(self.output_call_option_data['Delta'][self.center_index - 2]):
                        # 전체 보유 옵션을 돌리고
                        for mh in range(len(self.option_myhave['code'])):
                            # 풋일때만
                            if self.option_myhave['code'][mh][:3] == '201':
            ...
            # 차월물
                ...
                # 옵션의 델타가 선물의 델타보다 클때 [옵션 청산 매수]
                    ...
                    # 델타의 차이를 구하고
                    myhave_option_s_delta_diff = myhave_option_s_delta - myhave_future_s_delta_sum
                    # 옵션과 선물의 델타 차이를 비교하여 청산(매수)옵션의 델타보가 클때만 [콜이므로 -]
                    if myhave_option_s_delta_diff > abs(self.output_call_option_data_45['Delta'][self.center_index_45 - 2]):
                        # 전체 보유 옵션을 돌리고
                        for mh in range(len(self.option_myhave['code'])):
                            # 풋일때만
                            if self.option_myhave['code'][mh][:3] == '201':
    ...
    def option_s_center_index_change_ready(self):
        ...
        #     month_mall_type = 'center_index_45'
        # day_residue_int = 3
        # self.option_myhave = {'code': ['101V3000', '301V2337', '301V2340', '201V2342', '201V2340'],
        #                       'myhave_cnt': [4, 4, 3, 20, 10], 'sell_or_buy': [2, 1, 2, 1, 2]}
        # -----
        ...
        if myhave_sell_total_mall_basket_cnt_remove > 0:
            ...
            if myhave_sell_total_mall_basket_cnt_remove >= 3:
                ...
                if day_residue_int > 2:
                    ...
                    month_mall_type = 'center_index_45'
                    # center_index / center_index_45
                item_list_cnt_type = self.option_items_search(myhave_sell_total_mall_cnt, put_or_call, month_mall_type)
                # print(item_list_cnt_type)
                # item_list_cnt_type = {'code_no': ['301V2337'], 'cnt': [3], 'sell_buy_type': [2]}      # option_items_search 결과값
        ...
        # 선물 매수재고 => 콜청산(매수) 후 콜매도
            ...
            if myhave_buy_total_mall_basket_cnt_remove >= 3:
                ...
                if day_residue_int > 2:
                    ...
                    month_mall_type = 'center_index_45'
                    # center_index / center_index_45
                item_list_cnt_type = self.option_items_search(myhave_buy_total_mall_cnt, put_or_call, month_mall_type)
                # print(item_list_cnt_type)
                # item_list_cnt_type = {'code_no': ['301V2337'], 'cnt': [3], 'sell_buy_type': [2]}      # option_items_search 결과값
        ...
        if (myhave_sell_total_mall_basket_cnt_remove > 2) or (myhave_buy_total_mall_basket_cnt_remove > 2):
                ...
                if myhave_sell_current_mall_cnt > 0:
                    ...
                    if future_s_day_residue_int > (sell_roll_over_check_cnt + 2):
                        ...
                        if myhave_sell_total_mall_basket_cnt_remove >= 3:
                            ...
                            item_list_cnt_type = self.option_items_search(myhave_sell_total_mall_cnt, put_or_call,
                                                                              month_mall_type)
                            # print(item_list_cnt_type)
                            # item_list_cnt_type = {'code_no': ['301V2337'], 'cnt': [3], 'sell_buy_type': [2]}      # option_items_search 결과값
                ...
                if myhave_sell_current_mall_cnt > 0:
                    ...
                    if future_s_day_residue_int > (buy_roll_over_check_cnt + 2):
                        ...
                        if myhave_buy_total_mall_basket_cnt_remove >= 3:
                            ...
                            item_list_cnt_type = self.option_items_search(myhave_buy_total_mall_cnt, put_or_call,
                                                                          month_mall_type)
                            # print(item_list_cnt_type)
                            # item_list_cnt_type = {'code_no': ['301V2337'], 'cnt': [3], 'sell_buy_type': [2]}      # option_items_search 결과값
    ...
    def stock_trend_line_of_ai_day_data_fn(self):
        ...
        for db_name in db_name_lists:
            ...
            self.printt('stock_trend_line_of_ai_day 가장 최근 테이블 2건')
            self.printt(table_name_list)
            # print('stock_trend_line_of_ai_day 가장 최근 테이블 2건')
            # print(table_name_list)
